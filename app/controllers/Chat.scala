...
import actors.{ChatRoomActor, UserSocketActor}
import akka.actor._
...
class Chat @Inject()(val messagesApi: MessagesApi, system: ActorSystem) extends Controller with I18nSupport {
  ...
  lazy val chatRoom = system.actorOf(Props[ChatRoomActor], "chat-room")
  ...
  def socket = WebSocket.tryAcceptWithActor[JsValue, JsValue] { implicit request =>
    Future.successful(request.session.get(User) match {
      ...
      case Some(uid) => Right(UserSocketActor.props(chatRoom, uid))
    })
  }
}
